...
// Removed SliderPortfolioProps interface

function SliderPortfolio({ className, typeBg, parallax, tag: Tag = "div" }) {
  // Removed ': SliderPortfolioProps' annotation
  ...
  // Changed 'let tl' to a simple reference
  ...
  }, [parallax]); // Added parallax to dependency for Vite HMR stability
  ...
  const getContent = (swiper) => {
    ...
    return [newNum, oldNum, gsap.utils.selector(contentRef.current[newNum]), gsap.utils.selector(contentRef.current[oldNum])];
  };
  ...
  const swiperChange = (swiper) => {
    ...
    activeVideo(swiper);
    tl.progress(1);
    tl = gsap.timeline(); // Re-initializing timeline properly

    tl.fromTo($isRight ? oldTitle : oldTitle.reverse(), 1, animate.show, { ...animate.hide, onComplete: () => setActive(Number.parseInt(newNum, 10)) });
  ...
  return (
    <Tag className={dsnCN("root-slider half-content v-dark-head background-main", className)} ref={rootSlider}>
        ...
        <Swiper {...optionSwiper} grabCursor>
          {dataProject.map((item, key) => (
            <SwiperSlide className="overflow" key={key} data-dsn-id={key}>
              <BgImage className={"dsn-swiper-parallax-transform"} src={item.src} video={item.video} alt={item.title} overlay={item.overlay} height={"100%"} data-swiper-parallax={"30%"} />
            </SwiperSlide>
          ))}
      ...
      <NavSlider data={dataProject} speed={optionSwiper.speed} onSwiper={setNavSwiper} controller={{ control: bgSwiper }} />
      <SocialMediaSlider />
    </Tag>
  );
}
